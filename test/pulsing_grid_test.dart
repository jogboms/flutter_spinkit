...
void main() {
  group('PulsingGrid', () {
    testWidgets('needs either color or itemBuilder', (WidgetTester tester) async {
      ...
      expect(() => SpinKitPulsingGrid(color: Colors.white, itemBuilder: fakeBoxBuilder), throwsAssertionError);
    });
    ...
    testWidgets('needs itemBuilder to be non-null', (WidgetTester tester) async {
      expect(() => SpinKitPulsingGrid(itemBuilder: null), throwsAssertionError);
    });

    testWidgets('works with color', (WidgetTester tester) async {
      await tester.pumpWidget(createMaterialApp(const SpinKitPulsingGrid(color: Colors.white)));
    ...
    testWidgets('works with itemBuilder', (WidgetTester tester) async {
      await tester.pumpWidget(createMaterialApp(const SpinKitPulsingGrid(itemBuilder: fakeBoxBuilder)));
      expect(find.byType(SpinKitPulsingGrid), findsOneWidget);
    ...
    testWidgets('works without Material', (WidgetTester tester) async {
      await tester.pumpWidget(createWidgetsApp(const SpinKitPulsingGrid(color: Colors.white)));
      expect(find.byType(SpinKitPulsingGrid), findsOneWidget);
      expect(find.byType(DecoratedBox), findsWidgets);
      tester.verifyTickersWereDisposed();
    });
