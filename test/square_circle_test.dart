...
void main() {
  group("SquareCircle", () {
    ...
    testWidgets('works with itemBuilder', (WidgetTester tester) async {
      await tester.pumpWidget(createMaterialApp(SpinKitSquareCircle(itemBuilder: fakeBoxBuilder)));
      expect(find.byType(SpinKitSquareCircle), findsOneWidget);
    ...
    testWidgets('works without Material', (WidgetTester tester) async {
      await tester.pumpWidget(createWidgetsApp(SpinKitSquareCircle(color: Colors.white)));
      expect(find.byType(SpinKitSquareCircle), findsOneWidget);
      expect(find.byType(DecoratedBox), findsWidgets);
      tester.verifyTickersWereDisposed();
    });
